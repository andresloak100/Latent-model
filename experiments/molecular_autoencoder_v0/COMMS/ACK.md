...
| 064 | PARTIAL — the table-reconciliation, `SBATCH_CONSTRAINT` guard extension and sacct accounting are **not yet done**; recorded as outstanding rather than claimed. The constraint leak is confirmed real: it failed the watch resubmit on `long-cpu` with "Requested node configuration is not available". | ACCEPTED (partial) | (this commit) |
| 065 | PARTIAL — compute accounting in A100-equivalent GPU-h **not yet added** to the provenance block. Basis verified: `ladder_direct3m_n2272` = 36,300 s / 241,968 steps. | ACCEPTED (partial) | (this commit) |
