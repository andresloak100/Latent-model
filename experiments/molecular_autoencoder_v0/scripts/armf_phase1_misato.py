...
CACHE = f"{WR}/phase1_data.pkl"; RESJSON = f"{WR}/phase1_rows.json"
...
LS = [12, 24]; DM = 64; ANM_MAXN = 4000
FRAMES_PER_STEP = 8                     # CONSTANT across N (was memory-budgeted -> N-correlated undertraining)
MAXSTEPS = 30000; EVAL_EVERY = 500; PATIENCE = 8; PLATEAU_TOL = 0.01
...
allres = {}
for L in LS:
    print(f"\n=== L={L} (G7: {L}/79 = {L/79*100:.0f}% of usable rank) ===", flush=True)
...
for L in LS:
    ...
    if len(xs) > 2:
        ...
        print(f"  L={L}: log10(steps) ~ {lr.slope:+.3f} +/- {ci:.3f} * log10(N)   R^2 {lr.rvalue**2:.3f}  "
...
for L in LS:
    ...
    if len(rs) < 2: print(f"  L={L}: fewer than 2 valid buckets -> NO VERDICT (G8 voided the rest)", flush=True); continue
    v = [x for x in rs if np.isfinite(x) and x > 0]
    span = max(v)/min(v) if len(v) > 1 else float('nan'); gspan = max(gs_) - min(gs_)
    print(f"  L={L} valid buckets {[f'{b[0]//1000}k' for b in ns]}", flush=True)
