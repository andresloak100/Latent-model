...
Two modes, differing ONLY in the composition clause:
  ...
  ``--mode complex``
      at least ``--min-instances`` polymer instances IN THE DEPOSITED
      COORDINATES, OR at least one non-polymer entity -- i.e. assemblies *or*
      ligand-bearing entries. Both are admitted because the complex corpus is
      what exercises the two representation paths that single chains never
      touch (global res_pos across chains, and ordinal-slot ligand groups),
      and a set restricted to multi-chain-only would drop every
      monomer-plus-cofactor system.

      "In the deposited coordinates" is load-bearing: we download asymmetric
      units, and a biological assembly is usually generated by crystal
      symmetry rather than present in the file. See INSTANCE_COUNT_ATTR.
...
# The chain count that MATTERS is the one in the file we download.
#
# download_cif fetches {pdb_id}.cif -- the asymmetric unit. A biological
# assembly is a different thing: for most crystal forms the multimer is
# generated by symmetry operators and simply is not present in the deposited
# coordinates. Filtering on ``rcsb_assembly_info.polymer_entity_instance_count``
# therefore selects entries that are multimeric *in principle* and single-chain
# on disk -- it produced a corpus 42.5% multi-chain against a 72.4% reference,
# a gap no amount of resampling could close because it was above the arm's
# ceiling. ``deposited_polymer_entity_instance_count`` counts instances in the
# deposited coordinates, which is what the parser will actually see.
#
# It is also per-entry rather than per-assembly, so the two arms partition
# exactly (79,734 + 51,377 = 131,111 with no filter) instead of overlapping.
INSTANCE_COUNT_ATTR = "rcsb_entry_info.deposited_polymer_entity_instance_count"
...
def merge_pinned(pool, pinned, n, seed):
    ...
    Both lists are de-duplicated. The arms partition exactly now that they
    predicate on the per-entry deposited instance count, so this is no longer
    load-bearing for them -- but it still guards the ``--keep-list`` overlap
    and any future arm, and it cost nothing. It was load-bearing under the
    per-assembly attribute, where ~2.5% of the pool matched both ``>=2`` and
    ``<2`` and a concatenated build trained on those entries twice.
    """
    pinned = dedup(pinned)
