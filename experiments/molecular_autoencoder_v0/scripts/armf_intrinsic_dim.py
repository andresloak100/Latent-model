...
print("\n  *** SUPERSEDED (INBOX 002): 'a 1M-atom latent budget in the low hundreds' IS NOT AVAILABLE", flush=True)
print("  FROM THIS FILE, whichever way the frame-budget comparison lands. Every rank90 above is", flush=True)
print("  IN-SAMPLE, and an in-sample rank90 is a FLOOR: on ATLAS the same modes cover 77.1% of", flush=True)
print("  held-out variance (below 90% in 123/123 systems) and the out-of-sample count is 3.49x", flush=True)
print("  larger, with out-of-sample b >= 0.93. The frame-budget control here is necessary but NOT", flush=True)
print("  sufficient -- it catches censoring, not the in-sample bias, and the two push the SAME way.", flush=True)
print("  The 1e6-atom extrapolation is DROPPED, not re-planned; size DM from the codec's own held-out", flush=True)
print("  FVE-vs-DM saturation curve (armf_atlas_dm.py). See ROADMAP 'WIDTH CHAIN IS A FLOOR'. ***", flush=True)
