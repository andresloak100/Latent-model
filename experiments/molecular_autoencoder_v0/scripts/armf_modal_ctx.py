...
def coverage_report(rows, HO, hop_radius, log=print):
    ...
    log(f"  rank => reach is NOT SHOWN to be the constraint. A coverage slope whose CI merely spans")
    log(f"  zero does not license 'reach does not matter' -- if that CI also contains an effect large")
    log(f"  enough to explain the basis-quality gap, the arm is underpowered and NOTHING is licensed")
    log(f"  (Family C). Only a CI that EXCLUDES that effect would send us to rung 2. Tracking N")
