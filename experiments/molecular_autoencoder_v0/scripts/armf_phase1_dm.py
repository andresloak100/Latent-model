...
for DM_ in DMS:
    ...
    rows = []
    for d in he:
        v = fve(m, d, d["h"], d["T"]); ce = d["ceil"]; rs = realised_spectrum(m, d, L)
    ...
    torch.save(m.state_dict(), f"{WR}/ckpt_dm{DM_}.pt")
...
print("  *** RETRACTED (INBOX 002/012c). This block used to print: 'An N effect at L=1 is NOT a", flush=True)
print("  capacity limit -- rank90 and TICA both say intrinsic dimensionality does not grow with atom", flush=True)
print("  count.' THAT EXCLUSION IS RETIRED. rank90 was measured IN-SAMPLE (its own modes cover 77.1%", flush=True)
print("  of held-out variance, below 90% in 123/123 systems) and is CENSORED at large N, so it is a", flush=True)
print("  FLOOR, and out of sample it GROWS at b >= 0.93. Intrinsic dimensionality rising with N is", flush=True)
print("  exactly what a floor permits. CAPACITY IS A LIVE HYPOTHESIS and must be separated from the", flush=True)
print("  pooling/broadcast pathway (outcome B) BY THE DM SWEEP -- FVE still rising at the top width", flush=True)
print("  means information-limited, not pooling. Do not read an N effect here as evidence for B. ***", flush=True)
