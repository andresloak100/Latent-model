...
|---|---|---|---|
| **10307008** `warmup_ctl` | **Does the CURRENT procedure reproduce the PERSISTED arms?** LEGACY (warmup off, flat 30k budget) vs CURRENT (warmup 1000, lr-scaled), 2 seeds each, one process on one card, at n50/DM256/lr3e-4. Separates a procedure effect from seed noise before anything is invalidated. | `$WR/warmup_control.json` · `warmupctl_%j.log` | QUEUED, `long` + `--requeue`, 5 h |
