...
Generated 2026-08-08 12:22 · every number below carries the provenance of the run that produced it.
...
| code | git `94df5b54-dirty`, torch 2.13.0+cu130 |
...
> **Two trainings share the name `complex_d8`.** This section is the **3456-epoch** run (checkpoint sha256 `ca197bea6e912acb`). The committed `_true.pdb`/`_pred.pdb` pairs belong to a **900-epoch** run whose weights are not in the repo. Neither is wrong -- they are different models -- so quote this curve with its epoch count attached.
